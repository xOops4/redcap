...
class Auth_Container_LDAP extends Auth_Container
{
    ...
    function _connect()
    {
        ...
        // Change by Rob Taylor due to PHP 8.3 incompatibility - ldap_connect() can only take 1 argument
        $url = isset($this->options['host']) && $this->options['host'] != '' ? $this->options['host'] : ($this->options['url'] ?? "");
        $parts = parse_url($url);
        $scheme = $parts['scheme'] ?? null;
        $host = $parts['host'] ?? ($parts['path'] ?? $url);
        $port = isset($parts['port']) && isinteger($parts['port']) ? $parts['port'] : ($scheme === 'ldaps' ? 636 : 389);
        $scheme = $scheme ?? ($port === 636 ? 'ldaps' : 'ldap');
        $this->options['url'] = "{$scheme}://{$host}:{$port}";
    ...
    {
        $this->options['url']         = '';
        $this->options['host']        = ''; // Remove localhost as default
    ...
    function fetchData($username, $password, $isChallengeResponse=false)
    {
        ...
        if (@ldap_get_option($this->conn_id, LDAP_OPT_PROTOCOL_VERSION, $ver) && $ver == 3) {
            ...
            $username = utf8_encode_rc($username);
        }
