...
class Services_Twilio_TinyHttp {
  ...
  public function __call($name, $args) {
    ...
    $opts = $this->curlopts + array(
      ...
      CURLOPT_TIMEOUT => 60,
	  CURLOPT_SSL_VERIFYPEER => FALSE, // Added by Rob Taylor (4/17/2015) for compatibility reasons
	  CURLOPT_PROXY => PROXY_HOSTNAME, // Added by Rob Taylor (8/17/2015) for compatibility reasons
	  CURLOPT_PROXYUSERPWD => PROXY_USERNAME_PASSWORD, // Added by Rob Taylor (9/8/2015) for compatibility reasons
      ...
      if ($curl = curl_init()) {
        if (curl_setopt_array($curl, $opts)) {
          if ($response = curl_exec($curl)) {
            $parts = explode("\r\n\r\n", $response, 3);
			// Remove extra HTTP headers added by proxy hops // see https://github.com/twilio/twilio-php/issues/182
			while (strpos($parts[1], "HTTP") === 0) {
				array_shift($parts);
			}
            list($head, $body) = ($parts[0] == 'HTTP/1.1 100 Continue' ||  $parts[0] == 'HTTP/1.0 200 Connection established')  // Added by Rob Taylor (8/17/2015) for compatibility reasons
