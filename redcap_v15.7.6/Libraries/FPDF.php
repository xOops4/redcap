			...
			while($i<$nb)
			{
				...
				if($l>$wmax)
				{
					...
					if($sep==-1)
					{
						...
						{
							$this->ws=0;
                            if (method_exists("\PDF", "bottom_of_page") && $this->GetY() > \PDF::bottom_of_page) \PDF::new_page_check(0, $this, $GLOBALS['study_id_event'], true); // REDCap custom code - add new page
							$this->_out('0 Tw');
						}
						$this->Cell($w,$h,substr($s,$j,$i-$j),$b,2,$align,$fill);
					}
